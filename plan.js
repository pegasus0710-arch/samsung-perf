...
function SymbolPicker({BtnS, execCmd}){
  const [open,setOpen]=useState(false);
  const groups=[
    {g:"화살표", s:["→","←","↑","↓","↔","↕","⇒","⇐","⇑","⇓","⇔","▶","◀","▲","▼"]},
    {g:"기호",   s:["●","○","■","□","◆","◇","★","☆","♦","♣","♠","♥","•","‣","⁃"]},
          ...
          {groups.map(({g,s})=>(
            <div key={g} style={{marginBottom:6}}>
              ...
              <div style={{display:"flex",flexWrap:"wrap",gap:2}}>
                {s.map(sym=>(
                  <button key={sym}
                    onMouseDown={e=>{e.preventDefault();execCmd("insertText",sym);setOpen(false);}}
                    style={{width:26,height:26,border:`1px solid ${C.b1}`,
                      borderRadius:4,background:C.bg,cursor:"pointer",
                      fontSize:13,display:"flex",alignItems:"center",
                      justifyContent:"center",color:C.text,fontFamily:"inherit"}}
                    onMouseEnter={e=>e.currentTarget.style.background=C.card2}
  ...
  return(
    <div style={{borderRadius:8,border:"1px solid rgba(56,182,245,.3)",...style}}>
      ...
      <div style={{background:C.card2,borderBottom:`1px solid ${C.b1}`,
        ...
        {/* 목록 통합 드롭다운 */}
        <ListPicker BtnS={BtnS} execCmd={execCmd}/>
        <Sep/>

        {/* 특수기호 */}
        <SymbolPicker BtnS={BtnS} execCmd={execCmd}/>
        <Sep/>
      ...
      <div
        ...
        onContextMenu={e=>{
          ...
          e.preventDefault();
          // 기존 메뉴 제거
          document.querySelectorAll('.tbl-ctx-menu').forEach(m=>m.remove());
          const menu=document.createElement('div');
          menu.className='tbl-ctx-menu';
          Object.assign(menu.style,{
            position:'fixed',left:e.clientX+'px',top:e.clientY+'px',
            zIndex:9999,background:C.card,border:`1px solid ${C.b1}`,
            borderRadius:8,padding:'6px 0',minWidth:160,
            boxShadow:'0 4px 16px rgba(0,0,0,.2)',fontFamily:'inherit',
          });
          const items=[
            {l:'↑ 위에 행 추가', fn:()=>{
              const tr=td.closest('tr');
              const newTr=tr.cloneNode(true);
              newTr.querySelectorAll('td,th').forEach(c=>{c.innerHTML='';});
            ...
            {l:'← 왼쪽에 열 추가', fn:()=>{
              ...
              const ci=Array.from(td.closest('tr').children).indexOf(td);
              table.querySelectorAll('tr').forEach(row=>{
                const cell=document.createElement(row.rowIndex===0?'th':'td');
                cell.style.cssText=row.children[ci]?.style?.cssText||`border:1px solid ${C.b2};padding:6px 10px;min-width:60px;font-size:13px;color:${C.text};`;
            ...
            {l:'✕ 이 행 삭제', fn:()=>{
              const tr=td.closest('tr');
              if(tr.parentNode.rows.length>1){tr.remove();emit();}
            }, danger:true},
            {l:'✕ 이 열 삭제', fn:()=>{
              const table=td.closest('table');
              const ci=Array.from(td.closest('tr').children).indexOf(td);
              table.querySelectorAll('tr').forEach(row=>{
                if(row.children.length>1) row.children[ci]?.remove();
              });
              emit();
            }, danger:true},
            {l:'✕ 표 삭제', fn:()=>{
          ...
          items.forEach(item=>{
            if(item.fn===null){
              ...
              sep.style.cssText=`height:1px;background:${C.b1};margin:4px 0;`;
            ...
            Object.assign(btn.style,{
              ...
              color:item.danger?C.red:C.text,fontWeight:600,
              transition:'background .1s',
            });
            btn.onmouseenter=()=>btn.style.background=C.card2;
            btn.onmouseleave=()=>btn.style.background='';
            btn.onmousedown=ev=>{ev.preventDefault();item.fn();menu.remove();};
