/* ═══════════════════════════════════════════════════
   ...
   수정 범위: 실적 계산식·UI·기능 추가 시 이 파일만 교체
   ═══════════════════════════════════════════════════
   산출 기준:
...
const initData = () => ({
  "24": { perf: emptyM() },
  "25": { perf: emptyM(), target: emptyM() },
  "26": { perf: emptyM(), target: emptyM() },
});

// ═══════════════════════════════════════════════════
//  FORMATTERS
// ═══════════════════════════════════════════════════
const fmtN = v => gNum(v) > 0 ? gNum(v).toLocaleString() : "-";
...
function Spark({ vals, color = C.accent, h = 34 }) {
  const ns = vals.map(v => gNum(v));
  if (ns.every(v => v === 0)) return (
    <div style={{ height:h, display:"flex", alignItems:"center", justifyContent:"center" }}>
      <span style={{ color:C.muted, fontSize:9 }}>-</span>
    </div>
  );
  const W = 200, H = h, max = Math.max(...ns, 1);
  const pts = ns.map((v, i) => `${(i / Math.max(ns.length-1, 1))*W},${H - (v/max)*(H-4) - 2}`).join(" ");
  return (
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width:"100%", height:H }} preserveAspectRatio="none">
      ...
      {ns.map((v, i) => v > 0 && (
        <circle key={i} cx={(i/Math.max(ns.length-1,1))*W}
          cy={H - (v/max)*(H-4) - 2} r={2.5} fill={color} />
      ))}
    </svg>
  );
}
...
function InputTab({ data, setData }) {
  ...
  const hasTgt = yr !== "24";
  const pD = data[yr]?.perf    || emptyM();
  const tD = data[yr]?.target  || emptyM();
  ...
  const setP = useCallback((month, key, val) => {
    setData(prev => ({
      ...
      [yr]: {
        ...prev[yr],
        perf: { ...prev[yr].perf, [month]: { ...(prev[yr].perf?.[month]||{}), [key]: val } }
      }
    }));
  }, [setData, yr]);
  ...
  const setT = useCallback((month, key, val) => {
    ...
    setData(prev => ({
      ...
      [yr]: {
        ...prev[yr],
        target: { ...prev[yr].target, [month]: { ...(prev[yr].target?.[month]||{}), [key]: val } }
      }
    }));
  }, [setData, yr, hasTgt]);
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
          ...
          <div style={{ display:"flex", gap:4 }}>
            {[["single","월별 입력"],["bulk","전체 일괄"]].map(([v,l]) => (
              <button key={v} onClick={() => setMode(v)} style={{
                ...
                background: mode===v ? C.blue+"18" : "transparent",
                color: mode===v ? C.blue : C.muted,
              }}>{l}</button>
            ))}
          </div>
        </div>
      ...
      {mode === "single" ? (
          ...
          <div style={{ flex:1, background:C.card2, border:`1px solid ${C.border}`,
            ...
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14 }}>
              <div>
                ...
                <div style={{ color:C.muted, fontSize:11, marginTop:2 }}>20{yr}년 {MONTHS[mi]} · 억원</div>
              </div>
          ...
          <div style={{ width:224, display:"flex", flexDirection:"column", gap:12 }}>
                ...
                return (
                  <div key={dr.key} style={{ background:C.bg, border:`1px solid ${dr.c}28`,
                    ...
                    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                      ...
                      <span style={{ color:dr.c, fontSize:17, fontWeight:900 }}>
                    ...
                    {a !== null && tv > 0 && (
                      <div style={{ color:as.c, fontSize:10, fontWeight:700, marginTop:3 }}>
                        달성률 {as.t} · 목표 {tv.toLocaleString()}억
                      </div>
                    )}
                  </div>
                );
            ...
            <div style={{ background:C.card2, border:`1px solid ${C.border}`, borderRadius:12, padding:16 }}>
              ...
              <div style={{ color:C.muted, fontSize:10, marginBottom:10 }}>
                CE {CE > 0 ? CE.toLocaleString() : "-"}억 기준 (휴대폰 제외)
              </div>
              {[
                { key:"대외영업", label:"대외영업 (폰제외)" },
                { key:"혼수",    label:"혼수" },
                { key:"뉴홈",    label:"뉴홈" },
                { key:"SAC",    label:"SAC" },
                { key:"B2B",    label:"B2B" },
                ...
                const s = ceSharePct(r.key);
                return (
                  <div key={r.key} style={{ marginBottom:6 }}>
                    <div style={{ display:"flex", justifyContent:"space-between" }}>
                      <span style={{ color:C.muted2, fontSize:11 }}>{r.label}</span>
                      <span style={{ color:KC[r.key]||C.text, fontWeight:700, fontSize:12 }}>
                        {s ? s + "%" : "-"}
                      </span>
                    </div>
                    {s && <MiniBar pct={Number(s)} color={KC[r.key]||C.accent} />}
                  </div>
                );
              })}
            </div>
        ...
        <BulkInput yr={yr} hasTgt={hasTgt} pD={pD} tD={tD} setP={setP} setT={setT} />
      )}
    </div>
  );
}

function BulkInput({ yr, hasTgt, pD, tD, setP, setT }) {
  const inpRows = ALL_ROWS.filter(r => r.inp);
  return (
    <div style={{ background:C.card2, border:`1px solid ${C.border}`, borderRadius:12,
      ...
      <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:4 }}>
        <div style={{ color:C.text, fontWeight:800, fontSize:14 }}>
          20{yr}년 전체 일괄 입력
          <span style={{ color:C.muted, fontSize:11, fontWeight:400, marginLeft:8 }}>억원</span>
        </div>
        {hasTgt && (
          <div style={{ display:"flex", gap:6, fontSize:11 }}>
            <Chip color={C.orange}>실적 (상단)</Chip>
            <Chip color={C.blue}>목표 (하단)</Chip>
          </div>
        )}
      </div>
      <div style={{ color:C.muted, fontSize:11, marginBottom:14 }}>직접 입력 항목 · 대외영업/뉴홈/B2B는 자동계산</div>
                ...
                return (
                  <td key={monthIdx} style={{ padding:"3px 4px", textAlign:"center", verticalAlign:"middle" }}>
                    <div style={{ display:"flex", flexDirection:"column", gap:2, alignItems:"center" }}>
                      <input type="number" min="0" placeholder="0" value={pv}
                      ...
                      />
                      {hasTgt && (
                        <input type="number" min="0" placeholder="0" value={tv ?? ""}
                      ...
                      {a !== null && (
                        <span style={{ color:as.c, fontSize:9, fontWeight:700 }}>{as.t}</span>
                      )}
                    </div>
                  </td>
                );
...
function Dashboard({ data }) {
  const p26 = data["26"]?.perf   || emptyM();
  const p25 = data["25"]?.perf   || emptyM();
  const p24 = data["24"]?.perf   || emptyM();
  const t26 = data["26"]?.target || emptyM();
  const t25 = data["25"]?.target || emptyM();

  const lm26 = lastMi(p26);
  const lm25 = lastMi(p25);
  const emi  = lm26 >= 0 ? lm26 : (lm25 >= 0 ? lm25 : 11);
  ...
  const hp25ytd = sumRange(p25, "휴대폰", 0, emi);
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
      ...
      {/* ── YTD notice ── */}
      {lm26 >= 0 ? (
        <div style={{ background:C.card2, border:`1px solid ${C.green}40`,
          borderRadius:8, padding:"9px 16px", display:"flex", alignItems:"center", gap:8 }}>
          <span style={{ color:C.green, fontSize:14 }}>◉</span>
          <span style={{ color:C.muted2, fontSize:12 }}>
            기준: <strong style={{ color:C.text }}>26년 {MONTHS[emi]} 누계 (YTD)</strong>
        ...
        {mainKeys.map(key => {
          ...
          return (
            <div key={key} style={{ flex:"1 1 150px", background:C.card2,
              ...
              <div style={{ color:C.muted, fontSize:11, marginBottom:6, letterSpacing:"0.04em" }}>
                {key === "CE" ? "CE 전체" : key}
              </div>
              <div style={{ color:C.text, fontSize:22, fontWeight:900, marginBottom:5,
                letterSpacing:"-0.03em", fontVariantNumeric:"tabular-nums" }}>
                {v26 > 0 ? v26.toLocaleString() + "억" : <span style={{ color:C.muted }}>-</span>}
              </div>
              ...
              {v25 > 0 && (
                <div style={{ color:C.muted, fontSize:10, marginTop:5 }}>
                  전년 {v25.toLocaleString()}억 → 금년 {v26 > 0 ? v26.toLocaleString() : "-"}억
                </div>
              )}
            </div>
          );
            ...
            return (
              <div key={key} style={{ display:"flex", justifyContent:"space-between",
                ...
                <div style={{ display:"flex", gap:10, alignItems:"center" }}>
                  <span style={{ color:C.muted, fontSize:10, fontVariantNumeric:"tabular-nums" }}>
                    {v25>0?v25.toLocaleString():"-"} → {v26>0?v26.toLocaleString():"-"}
                  </span>
              ...
              <div key={key} style={{ marginBottom:9 }}>
                ...
                {tv > 0 && (
                  <div style={{ color:C.muted, fontSize:10, marginTop:2 }}>
                    {pv>0?pv.toLocaleString():"-"}억 / 목표 {tv.toLocaleString()}억
                  </div>
                )}
              </div>
            );
      ...
      <div style={{ background:C.card2, border:`1px solid ${C.border}`, borderRadius:12, padding:20 }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14 }}>
          <div>
            <div style={{ color:C.text, fontWeight:800, fontSize:14 }}>CE 월별 실적 추이</div>
          ...
          {monthlyBars.map((d, i) => (
            <div key={i} style={{ flex:1, display:"flex", gap:1, alignItems:"flex-end" }}>
              {[
                ...
                return (
                  <div key={bi} title={`${MONTHS[i]} ${["24실","25실","26실","26목"][bi]}: ${b.v.toLocaleString()}억`}
            ...
            return (
              <div key={key} style={{ background:C.card, borderRadius:9, padding:"12px 14px",
                ...
                <div style={{ display:"flex", gap:8, marginBottom:4 }}>
                  <span style={{ color:C.muted, fontSize:10 }}>25년 {tot25>0?tot25.toLocaleString():"-"}억</span>
                  <span style={{ color:C.text, fontSize:10, fontWeight:600 }}>26년 {tot26>0?tot26.toLocaleString():"-"}억</span>
                </div>
                <svg viewBox={`0 0 200 36`} style={{ width:"100%", height:36 }} preserveAspectRatio="none">
                  {(() => {
                    const mkPts = vals =>
                      vals.map((v,i) => `${(i/11)*200},${36-(v/maxV)*32-2}`).join(" ");
                    return (
                      <>
...
function Analysis({ data }) {
  ...
  const pD  = data[selYr]?.perf   || emptyM();
  const tD  = data[selYr]?.target || emptyM();
  const prevYr = selYr === "26" ? "25" : selYr === "25" ? "24" : null;
  const prevP  = prevYr ? (data[prevYr]?.perf || emptyM()) : null;
  ...
  const TH = ({ children, right = false, accent = false }) => (
    <th style={{ padding:"7px 8px", textAlign:right?"right":"left", color:accent?C.accent:C.muted,
      fontWeight:600, fontSize:11, whiteSpace:"nowrap" }}>{children}</th>
  );
  const TD = ({ children, right = false, color, bold = false }) => (
    <td style={{ padding:"6px 8px", textAlign:right?"right":"left",
      color:color||C.text, fontWeight:bold?700:400, fontSize:12 }}>{children}</td>
  );
  const RowLabel = ({ row }) => (
    <td style={{ padding:"6px 12px", paddingLeft:12+row.lv*14,
      color:row.bold?(KC[row.key]||C.text):C.muted2,
      fontWeight:row.bold?700:400, fontSize:12,
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
      ...
      {lm >= 0 && (
        <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:8,
          ...
          ◉ 데이터 기준: 20{selYr}년 <strong style={{ color:C.text }}>{MONTHS[emi]}</strong> 누계
      ...
      {view === "table" && (
        <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12,
          ...
          <div style={{ color:C.text, fontWeight:800, fontSize:14, marginBottom:14 }}>
            실적 현황 <span style={{ color:C.muted, fontSize:12, fontWeight:400 }}>(단위: 억원)</span>
          </div>
            ...
            <tbody>
              {analysisRows.map(row => (
                <tr key={row.key} style={{ borderBottom:`1px solid ${C.border}18`,
                  ...
                  <RowLabel row={row} />
                  {monthRows.map((r, mi) => {
                    const v = gNum(r[row.key]);
                    return <TD key={mi} right color={v>0?C.text:C.muted}>{v>0?v.toLocaleString():"-"}</TD>;
                  })}
                  <TD right color={KC[row.key]||C.accent} bold>
                    {rowTotal(row.key) > 0 ? rowTotal(row.key).toLocaleString() : "-"}
                  </TD>
                </tr>
              ))}
            </tbody>
                ...
                {["CE","대외영업","혼수","뉴홈","SAC","B2B","SMB","농협","휴대폰"].map(key => {
                  ...
                  return (
                    <tr key={key} style={{ borderBottom:`1px solid ${C.border}18` }}>
                      ...
                      {MONTHS.map((_, mi) => {
                        const p = gNum(monthRows[mi][key]);
                        const t = gNum(tMonthRows[mi][key]);
                          ...
                          <td key={mi} style={{ padding:"6px 8px", textAlign:"right" }}>
                            {t > 0 ? (
                              <div style={{ display:"flex", flexDirection:"column", alignItems:"flex-end", gap:1 }}>
                                <span style={{ color:as_.c, fontSize:11, fontWeight:700 }}>{as_.t}</span>
                                <span style={{ color:C.muted, fontSize:9 }}>{p>0?p.toLocaleString():"-"}/{t.toLocaleString()}</span>
                              </div>
                      ...
                      <td style={{ padding:"6px 12px", textAlign:"right" }}>
                        ...
                        {ytdT > 0 && (
                          <div style={{ color:C.muted, fontSize:9, marginTop:1 }}>
                            {ytdP>0?ytdP.toLocaleString():"-"}/{ytdT.toLocaleString()}
                          </div>
                        )}
                      </td>
                    </tr>
              ...
              return (
                <div key={key} style={{ flex:"1 1 150px", background:C.card2,
                  ...
                  <div style={{ color:C.muted, fontSize:11 }}>
                    {ytdP>0?ytdP.toLocaleString():"-"}억 / {ytdT>0?ytdT.toLocaleString():"-"}억
                  </div>
      ...
      {view === "achieve" && selYr === "24" && (
        <div style={{ background:C.card, borderRadius:12, padding:30, textAlign:"center", color:C.muted }}>
          24년은 목표 데이터가 없습니다. 25년 또는 26년을 선택하세요.
        </div>
      )}
      ...
      {view === "growth" && (
        <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12,
          padding:20, overflowX:"auto" }}>
          <div style={{ color:C.text, fontWeight:800, fontSize:14, marginBottom:4 }}>
            전년비 성장률 — 20{selYr}년 vs {prevYr ? "20"+prevYr+"년" : "기준없음"}
          </div>
                ...
                {analysisRows.map(row => {
                  ...
                  return (
                    <tr key={row.key} style={{ borderBottom:`1px solid ${C.border}18`,
                      ...
                      <RowLabel row={row} />
                      {MONTHS.map((_, mi) => {
                        const cv = gNum(monthRows[mi][row.key]);
                        const pv = gNum(prevRows[mi][row.key]);
                        const gr = gRate(cv, pv); const gs = gStyle(gr);
                        return (
                          <td key={mi} style={{ padding:"6px 8px", textAlign:"right" }}>
                            {(cv>0||pv>0) ? <span style={{ color:gs.c, fontSize:11, fontWeight:600 }}>{gs.t}</span>
                              : <span style={{ color:C.muted }}>-</span>}
                          </td>
                        );
                      })}
          ...
          <table style={{ borderCollapse:"collapse", minWidth:900 }}>
            ...
            <tbody>
              {["대외영업","혼수","뉴홈","SAC","B2B","SMB","농협","휴대폰"].map(key => {
                const totCE = rowTotal("CE", pD);
                const totHP = rowTotal("휴대폰", pD);
                ...
                return (
                  <tr key={key} style={{ borderBottom:`1px solid ${C.border}18` }}>
                    ...
                    {MONTHS.map((_, mi) => {
                      const ce  = gNum(monthRows[mi].CE);
                      const hp  = gNum(monthRows[mi].휴대폰);
      ...
      {view === "trend" && (
        <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
          ...
          <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:18 }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))", gap:10 }}>
              {["CE","대외영업","혼수","뉴홈","SAC","B2B","SMB","농협","휴대폰"].map(key => {
                ...
                const totP = prev ? prev.reduce((a,b)=>a+b,0) : 0;
          ...
          <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12,
            ...
            <div style={{ color:C.muted, fontSize:11, marginBottom:14 }}>
              20{selYr}년 vs 20{prevYr||"-"}년 동월 (녹색:성장/적색:감소)
            </div>
            {prevRows ? (
              <table style={{ borderCollapse:"collapse", minWidth:700 }}>
                <thead>
                  <tr>
                    ...
                    {MONTHS.map(m => <th key={m} style={{ padding:"5px 8px", textAlign:"center", color:C.muted, fontWeight:600, minWidth:46 }}>{m}</th>)}
                  </tr>
                </thead>
                <tbody>
                  {["CE","대외영업","혼수","뉴홈","SAC","B2B","SMB","농협","휴대폰"].map(key => (
  ...
  return (
    <div style={{ minHeight:"100vh", background:C.bg, color:C.text,
      ...
      <div style={{ background:C.surface, borderBottom:`1px solid ${C.border}`,
        ...
        <div style={{ display:"flex", alignItems:"center", height:54, gap:24, maxWidth:1360, margin:"0 auto" }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:10, flexShrink:0 }}>
            <div style={{ width:30, height:30,
              ...
              fontSize:15, fontWeight:900, color:"#fff", boxShadow:`0 0 12px ${C.accent}50` }}>S</div>
            <div>
              <div style={{ color:C.text, fontWeight:900, fontSize:13, letterSpacing:"-0.03em" }}>
                Chungcheong Sales
              </div>
              <div style={{ color:C.muted, fontSize:9, letterSpacing:"0.06em" }}>충청영업팀 실적관리 시스템</div>
            </div>
          </div>
          ...
          <nav style={{ display:"flex", gap:2 }}>
            {TABS.map(t => (
              <button key={t.k} onClick={() => setTab(t.k)} style={{
                ...
                background: tab===t.k ? C.accent+"20" : "transparent",
      ...
      <div style={{ maxWidth:1360, margin:"0 auto", padding:"22px 28px" }}>
        ...
        <div style={{ marginBottom:18 }}>
          ...
          <p style={{ margin:"4px 0 0", color:C.muted, fontSize:12 }}>
            ...
            {tab==="input"     && "24년/25년/26년 월별 실적·목표 수기 입력 · 자동계산 · 실시간 저장"}
          </p>
        </div>
