...
function TargetUnlockModal({onSuccess,onClose,title="목표 입력 잠금 해제",hashToMatch=TGT_PW_HASH}){
  ...
  return(
    <div style={{position:"fixed",inset:0,zIndex:900,background:"rgba(0,0,0,.7)",
      display:"flex",alignItems:"center",justifyContent:"center",backdropFilter:"blur(4px)"}}>
      <div style={{
        background:C.card,border:`1px solid ${C.blue}50`,borderRadius:16,
        padding:"32px 36px",textAlign:"center",maxWidth:320,width:"90%",
        ...
        <input
          ...
          placeholder="••••••••"
          style={{width:"100%",background:C.bg,border:`1px solid ${err?C.red:C.b2}`,
            borderRadius:8,padding:"10px 14px",color:C.text,fontSize:15,
            letterSpacing:2,outline:"none",fontFamily:"inherit",
            transition:"border-color .2s",textAlign:"center"}}/>
        {err&&<div style={{color:C.red,fontSize:11,marginTop:6}}>{err}</div>}
        <div style={{display:"flex",gap:8,marginTop:14}}>
          <button onClick={onClose} style={{flex:1,padding:"9px",border:`1px solid ${C.b2}`,
            borderRadius:8,background:"transparent",color:C.muted,
            cursor:"pointer",fontWeight:700,fontSize:12,fontFamily:"inherit"}}>
            취소
          </button>
          <button onClick={tryUnlock} style={{flex:2,padding:"9px",border:"none",
            borderRadius:8,background:C.blue,color:"#fff",
            cursor:"pointer",fontWeight:700,fontSize:12,fontFamily:"inherit"}}>
            잠금 해제
          </button>
        </div>
...
function InputTab({data,setData,mode,onSave,saveState,hasUnsaved,dbStatus,onImport,isTargetUnlocked,onRequestTargetUnlock,onTargetLock,theme}){
  const dbLoadFailed = dbStatus&&(dbStatus.startsWith("❌")||dbStatus.includes("실패")||dbStatus.includes("재시도↻")||dbStatus.startsWith("🔄"))&&!dbStatus.startsWith("✅")&&!dbStatus.startsWith("⚠ 문서없음");
        ...
        <div style={{
          ...
          <div style={{display:"flex",alignItems:"center",gap:6}}>
            ...
            <NumInput value={row.auto?pv:pD[sk(mi)]?.[row.key]}
              readOnly={row.auto||!isTargetUnlocked} color={isTargetUnlocked?row.gc||mColor:C.muted}
              ...
              <div style={{marginLeft:"auto",display:"flex",gap:4,alignItems:"center"}}>
                {isTargetUnlocked?(
                  ...
                  <button onClick={onRequestTargetUnlock} style={{
                    ...
                    border:`1px solid ${C.orange}`,background:C.orange+"18",
                    color:C.orange,cursor:"pointer",fontSize:10,fontWeight:700,fontFamily:"inherit"}}>
                    🔓 잠금 해제
                  </button>
                )}
              </div>
                ...
                <tbody>
                  {INPUT_ROWS.map((row,ri)=>(
                    <React.Fragment key={row.key}>
                      ...
                      <BulkDataRow row={row} type="perf" perfLocked={!isTargetUnlocked}/>
                    </React.Fragment>
                  ))}
                </tbody>
              </table>
            ...
            <div style={{marginLeft:"auto"}}>
              {isTargetUnlocked?(
                ...
                <button onClick={onRequestTargetUnlock} style={{
                  ...
                  padding:"5px 12px",borderRadius:7,cursor:"pointer",fontFamily:"inherit",
                  fontWeight:700,fontSize:11,
                  border:`1px solid ${C.orange}60`,background:C.orange+"15",color:C.orange,
                  transition:"all .15s"}}
                  onMouseEnter={e=>{e.currentTarget.style.borderColor=C.orange;e.currentTarget.style.background=C.orange+"25";}}
                  onMouseLeave={e=>{e.currentTarget.style.borderColor=C.orange+"60";e.currentTarget.style.background=C.orange+"15";}}>
                  🔒 <span>잠금 해제</span>
                </button>
              )}
            </div>
...
function ReportModal({onClose, mode, tab}){
  ...
  const downloadExcelImg = async() => {
    ...
    try{
      ...
      const a = document.createElement("a");
      a.href = canvas.toDataURL("image/png");
      a.download = `충청_레포트_${mode}_${new Date().toISOString().slice(0,10)}.png`;
      a.click();
  ...
  const downloadExcelData = async() => {
    try{
      ...
      const keys = ["CE","대외영업","혼수","뉴홈","입주","이사","SAC","거주중","B2B","SMB","농협","휴대폰"];
      const calcVal = (m,k)=>{
        if(k==="대외영업") return ["혼수","입주","이사","SAC","거주중","SMB","농협","휴대폰"].reduce((s,kk)=>s+(parseFloat(m[kk])||0),0);
      ...
      ["26","25","24"].forEach(yr=>{
        [mode].forEach(md=>{
          const pD = reportData[yr]?.[md]?.perf||{};
          const tD = reportData[yr]?.[md]?.target||{};
          const rows = [["항목",...months,"연간합계"]];
      ...
      XLSX.writeFile(wb,`충청_실적데이터_${mode}_${new Date().toISOString().slice(0,10)}.xlsx`);
      setMsg("✅ 엑셀 저장 완료");
    }catch(e){ setMsg("❌ 오류: "+e.message); }
  };

  const BtnRow=({icon,label,desc,onClick,c,badge})=>(
    <button onClick={onClick} style={{
      display:"flex",alignItems:"center",gap:14,width:"100%",padding:"14px 16px",
      borderRadius:9,cursor:"pointer",textAlign:"left",fontFamily:"inherit",
      border:`1px solid ${c}40`,background:c+"0d",marginBottom:8,
      transition:"background .15s"}}
      ...
      <div style={{flex:1}}>
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          ...
          {badge&&<span style={{fontSize:9,fontWeight:700,padding:"2px 6px",borderRadius:3,
            background:c+"22",color:c,border:`1px solid ${c}40`}}>{badge}</span>}
        </div>
        <div style={{color:C.muted,fontSize:11,marginTop:3,whiteSpace:"pre-line"}}>{desc}</div>
      </div>
    </button>
  );
    ...
    <div style={{position:"fixed",inset:0,zIndex:1000,background:"rgba(0,0,0,.75)",
      ...
      <div style={{background:C.card,border:`1px solid ${C.b2}`,borderRadius:16,
        ...
        <div style={{padding:"16px 20px",borderBottom:`1px solid ${C.b1}`,
          display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div>
            <div style={{fontWeight:800,fontSize:15,color:C.text}}>🖨️ 레포트 출력</div>
            <div style={{color:C.muted,fontSize:11,marginTop:2}}>{mode} · {getLabel()} 화면 기준</div>
          </div>
          <button onClick={onClose} style={{background:"transparent",border:"none",
            color:C.muted,cursor:"pointer",fontSize:18}}>✕</button>
        </div>
    ...
    <div key={themeKey} style={{minHeight:"100vh",background:C.bg,color:C.text,
      ...
      <div style={{position:"sticky",top:0,zIndex:300,background:C.surf,
        ...
        <div style={{maxWidth:1360,margin:"0 auto",display:"flex",alignItems:"center",
          gap:2,padding:"6px 12px",overflow:"hidden"}}>
          {/* 로고 */}
          <a href="index.html" style={{display:"flex",alignItems:"center",gap:6,
            textDecoration:"none",color:C.text,marginRight:4,flexShrink:0}}>
            <div style={{width:26,height:26,borderRadius:7,
              background:`linear-gradient(135deg,${mColor},${C.accent})`,
              display:"flex",alignItems:"center",justifyContent:"center",
              fontWeight:900,fontSize:13,color:"#fff",flexShrink:0}}>C</div>
            {!isMobile&&(
              <div>
          ...
          {/* 우측: 테마 + (PC전용: dbStatus, 버전, 줌, 다운로드) */}
          <div style={{marginLeft:"auto",display:"flex",alignItems:"center",gap:isMobile?4:8,flexShrink:0}}>
            {/* DB 연결 상태 — PC만 */}
            {!isMobile&&<span style={{fontSize:10,fontWeight:600,
            ...
            <button onClick={toggleTheme} style={{
              padding:isMobile?"5px 8px":"4px 10px",borderRadius:6,cursor:"pointer",
              fontFamily:"inherit",fontWeight:700,fontSize:12,border:`1px solid ${C.b1}`,
            ...
            {/* 버전 — PC만 */}
            {!isMobile&&<span style={{color:C.muted,fontSize:9}}>{APP_VER}</span>}
            {/* 줌 — PC만 */}
            {!isMobile&&<div style={{display:"flex",alignItems:"center",gap:3,background:C.card2,
            ...
            <button onClick={()=>setShowBackupMain(true)} style={{
              padding:isMobile?"5px 8px":"4px 12px",borderRadius:7,
              border:`1px solid ${C.teal}40`,
              background:C.teal+"10",color:C.teal,fontWeight:700,
        ...
        {/* ─ 행2: 판매/매출 + (모바일: dbStatus) ─ */}
        <div style={{background:C.surf,borderTop:`1px solid ${C.b1}22`,padding:"0 12px"}}>
          <div style={{maxWidth:1360,margin:"0 auto",display:"flex",alignItems:"center",
            height:38,gap:8}}>
              ...
              onClick={()=>{
                if(dbStatus.includes("재시도↻")){
                  ...
                  const retry=async()=>{
                    while(retries>=0){
                      try{
                        const snap=await Promise.race([DOC().get(),new Promise((_,rej)=>setTimeout(()=>rej(new Error("timeout")),12000))]);
                        if(snap.exists){const raw=snap.data().perfData;const loaded=migrate(raw);const ce24=loaded?.["24"]?.["매출"]?.perf?.["0"]?.CE;setDbStatus(gNum(ce24)>0?"✅ 로드완료":"✅ 연결됨");setData(loaded);localStorage.setItem("cst_v13",JSON.stringify(loaded));}
                        else{setDbStatus("⚠ 문서없음");}
                        return;
                      }catch(e){retries--;if(retries<0){setDbStatus(e.message==="timeout"?"⚠ 연결지연 — 재시도↻":"❌ 로드실패 — 재시도↻");}else{setDbStatus(`⚠ 오류`);await new Promise(r=>setTimeout(r,2000));}}
                    }
                  };retry();
                }
              }}>
